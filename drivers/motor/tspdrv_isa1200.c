...
static int isa1200_parse_dt(struct device *dev)
{
	...
	vibrator_drvdata.sda = of_get_named_gpio(np, "isa1200,sda-gpio",0);
...
#if defined(CONFIG_MACH_MATISSELTE_VZW)
	gpio_tlmm_config(GPIO_CFG(vibrator_drvdata.sda,  0, GPIO_CFG_INPUT, GPIO_CFG_NO_PULL, GPIO_CFG_2MA),GPIO_CFG_ENABLE);
#endif
